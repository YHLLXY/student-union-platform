...
| [经验教训](./docs/lessons-learned-phase1-5.md) | Phase 1-8 开发经验总结（19 条） → 知识库经验笔记见本目录（vault `30-项目/学生会交流平台/`） |
...
| 日期 | 版本 | 内容 |
|------|------|------|
| 2026-07-26 | v3.5 | **移动端专项优化** — 安全区适配 + 视口修正、手机搜索入口恢复、3档断点体系(480/768/1024)、看板横向滑动snap、全局按压反馈、8模块断点补齐、小屏Header优化、恢复缩放权限 |
| 2026-07-26 | v3.4 | **UI 全面优化** — 精致极简风格、CSS 变量体系（50+变量统一17文件）、Header 深色渐变、登录页毛玻璃重设计、骨架屏+路由动画+Bell摇晃动效注入、平板断点增强、prefers-reduced-motion 支持 |
| 2026-07-26 | v3.3 | **质量加固** — 抢票 RPC 原子化（FOR UPDATE 行锁防超卖）、helpers 拆分（dateUtils/roleUtils/fileUtils）+ dayjs 替换、邀请码增强（max_uses/expires_at/revoked_at）、Storage DELETE 策略修正（仅所有者可删）、密码强度校验（≥8位+字母+数字）、ADR 架构决策记录 + 部署交接指南、SW 版本检测优化（10min轮询 + changelog联动） |
