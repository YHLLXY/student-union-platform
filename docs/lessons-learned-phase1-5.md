...
PWA 功能做完后，用户反馈"没有安装按钮"。排查发现，安装按钮组件被 `{md && <PwaInstallButton />}` 的条件包裹——`md` 是 Ant Design 的 `Grid.useBreakpoint()`，≥768px 才为 true。Android 手机屏幕宽度 ≤412px，`md` 永远为 false，**安装按钮在手机上根本不渲染**。

### 根因

开发时习惯性地把新按钮和旁边的"功能指南""反馈建议"按钮用同样的 `{md && ...}` 条件包裹。但功能指南和反馈在手机上确实意义不大（小屏空间珍贵），而 **PWA 安装的主战场恰恰是手机**——桌面端的 Chrome 虽然也支持 PWA，但用户极少在桌面端使用这个功能。

### 解决方案

去掉 `{md && ...}` 条件限制，安装按钮在移动端和桌面端均显示。

### 教训

1. **PWA 相关 UI 必须移动端优先。** 安装按钮、更新提示、离线指示器——这些功能的核心用户在手机上，不应加桌面端断点限制。
2. **`beforeinstallprompt` 事件不可靠，必须有手动兜底。** 浏览器不一定每次都自动弹出安装横幅（触发条件由 Chrome 内部控制），手动安装按钮是必要的。
3. **写 `{md && ...}` 前想清楚：这个功能对手机用户有没有价值？** 很多时候答案和直觉相反。

**相关文件：** [PwaInstallButton.tsx](../src/components/PwaInstallButton.tsx) | [AppLayout.tsx](../src/components/AppLayout.tsx)
**Commit:** `d70516b1`

---

## 十八、版本更新通知失效 — 忘记更新 version.json

### 问题

连续两次修复推送后（安装按钮 + 移动端弹窗溢出），用户反馈没有收到版本更新通知。排查发现 `public/version.json` 的版本号始终是 `3.0.0`，从未被更新。版本通知系统依赖 `version.json` 中版本号变化来触发公告弹窗，版本号不变 → 不弹通知。

### 根因

开发流程中缺少"更新 version.json"这一步。PWA 的版本通知链条是：

```
version.json 版本号变化 → SW 检查发现新版本 → localStorage 写入新版本数据
→ React 挂载后读取 → antd notification 弹窗
```

这个链条的起点是 `version.json`，如果不更新它，整条链都不会触发。

### 教训

1. **每次面向用户的部署，必须同时更新 `public/version.json`**（版本号 + changelog）和 `public/sw.js` 中的 `CACHE_VERSION`。这是 PWA 版本通知的起点，漏掉它等于没有通知。
2. **将此步骤加入 CLAUDE.md 开发流程。** 在"功能验证 → 提交推送"之间插入"更新版本号"步骤。
3. **SW 缓存版本号建议与 version.json 保持一致**，形成一一对应关系，方便追溯。

**相关文件：** [public/version.json](../public/version.json) | [public/sw.js](../public/sw.js)
**Commit:** `65c6b733`

---

## 十九、CSS max-width 修不了 Popover 溢出——必须换组件

### 问题

通知铃铛在手机上用了两次修复仍然溢出。第一次加 `max-width: calc(100vw - 48px)`，第二次加 `overlayInnerStyle` 双保险，均无效。

### 根因

Ant Design 的 `Popover` 组件底层用 `@floating-ui/react` 做绝对定位。流程是：

```
floating-ui 读取内容宽度（360px）
  → 计算弹窗应该出现在屏幕哪个坐标
  → 把弹窗的 DOM 绝对定位钉死在那里
```

CSS `max-width` 只能把内容变窄，但**弹窗的定位原点已经被 floating-ui 算好了**——盒子锚定在屏幕外的位置，里面内容再窄也看不到。这就像把一张大桌子搬到墙边，墙挡住了桌面的一半，你把桌上的东西挪近身，但桌子本身还是被墙卡着。

### 解决方案

**移动端用 `<Drawer>` 替代 `<Popover>`。** Drawer 是 `position: fixed` + CSS transition，宽度用 `90vw` 这种相对单位，天然不会溢出视口。

桌面端保留 Popover（屏幕够宽，360px 不存在溢出问题）。

### 教训

1. **JS 定位组件（Popover、Tooltip、Dropdown）的溢出问题，CSS max-width 修不了。** 必须在 JS 层面解决——要么换组件（Drawer），要么在 floating-ui 的 middleware 里加约束。
2. **移动端弹窗 UX：Drawer > Popover。** 手机上 Drawer 从侧边滑入是原生 App 标配体验，Popover 悬浮小三角更适合桌面端鼠标交互。
